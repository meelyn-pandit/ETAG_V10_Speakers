...
const String RFIDUser = "ELIB123";    // Your unique user ID for ETAG
...
unsigned int stopCycleCount = 1500;     // How many read cycles to maintain serial comminications
...
void setup() {  // setup code goes here, it is run once before anything else
  ...
  pinMode(MP3Pwr1, OUTPUT);       // pin that powers the GPS via the motor controller
  digitalWrite(MP3Pwr1, LOW);     // turn off
  ...
  rtc.updateTime();
  String logStr = "Logging started at " + showTime2();
...
//Check to see if it is time to execute nightime sleep mode.
  ...
  if(Debug) serial.println(showTime2());                        // Show the current time 
  ...
  if (curTimeHHMM == slpTime) {                                // Check to see if it is sleep time
     String SlpStr =  "Entering sleep mode at " + showTime2();  // if it's time to sleep make a log message
     ...
     SlpStr =  "Wake up from sleep mode at " + showTime2();     // log message
     if(SDOK == 1) {SDwriteString(SlpStr, logFile);}           // save log message if SD writes are enabled
  }
...
//Try to read tags - if a tag is read and it is not a recent repeat, write the data to the SD card and the backup memory.
  ...
  if (FastRead(RFcircuit, checkTime, pollTime1) == 1) {
    ...
    String SDsaveString = String(RFIDUser) + "-" + String(deviceID) + ", " + String(RFIDstring)  + ", " + showTime2() + ", " + RFcircuit;    // Create a data string for SD write
    ...
    if(currRFID != pastRFID  | (HHMMSS-pastHHMMSS >= delayTime)) {                      // See if the tag read is a recent repeat
      if(Debug) serial.println("Tag detected, saving data");
      ...
      writeFlashArray(fAddress, flashData, 12);  //write array 
      fAddress = advanceFlashAddr(fAddress, 12);                                        // Add 12 to the flash address 
  ...
  playWithVolume(0X1E01);//play the first song with volume 15(0x0F) class
 }
...
String showTime3() { //Get date and time strings and cobine into one string
  currentDateTime = "20" + String(rtc.getYear()) + "-" + String(rtc.getMonth()) + "-" + String(rtc.getDate()) + " " +
        String(rtc.getHours()) + ":" + String(rtc.getMinutes()) + ":" + String(rtc.getSeconds());
  return currentDateTime;
}


String showTime2() { //Get date and time strings and cobine into one string
   char cDateTime[19];
   sprintf(cDateTime, "20%02d-%02d-%02d %02d:%02d:%02d",
         rtc.getYear(), rtc.getMonth(), rtc.getDate(), rtc.getHours(), rtc.getMinutes(), rtc.getSeconds());
  currentDateTime = String(cDateTime);
  return currentDateTime;
}

//String showTime4() { //Get date and time strings and cobine into one string
//
//     char BA[6];  
//   BA[0] = rtc.getYear();
//   BA[1] = rtc.getMonth();
...
//   BA[4] = rtc.getMinutes();
//   BA[5] = rtc.getSeconds();
//   sprintf(currentDateTime, "20%02d-%02d-%02d %02d:%02d:%02d",
...
void dumpMem(byte doAll) {                        // Display backup memory
  ...
  unsigned long fAddressEnd = fAddress;           // last address for stored data
  ...
  String UserDev =  String(RFIDUser) + "-" + String(deviceID) + ", ";
  ...
  while (curAddr < fAddressEnd) {                 // Escape long memory dump by entering a character
    ...
    sprintf(dataLine, "%02X%02X%02X%02X%02X, 20%02d-%02d-%02d %02d:%02d:%02d, %d",
         BA[0], BA[1], BA[2], BA[3], BA[4], BA[8], BA[6], BA[7], BA[9], BA[10], BA[11], BA[5]);
    //   RFID1  RFID2  RFID3  RFID4  RFID5  Year   Month  Day    Hour   Min     Sec     Antenna  
    serial.print(UserDev);
...
void writeMem(byte doAll) {
  ...
  uint32_t fAddrEnd;
  doAll == 0 ? fAddrEnd = fAddress : fAddrEnd = ((8192 << 10) + 512) ;   // get last flash address
  ...
  while(curPAddr <= pAddr) {  //read in full pages one at a time.
     ...
     if (dataFile) {
         for (int i = 0; i < lastBAddr; i=i+12) {             // loop to read in an RFID code from the flash and send it out via serial comm
            ...
            sprintf(charRFID, "%02X%02X%02X%02X%02X", 
                mBytes[i], mBytes[i+1], mBytes[i+2], mBytes[i+3], mBytes[i+4]); 
            static char dateAndTime[17];
            sprintf(dateAndTime, "20%02d-%02d-%02d %02d:%02d:%02d",
                mBytes[i+8], mBytes[i+6], mBytes[i+7], mBytes[i+9], mBytes[i+10], mBytes[i+11]);  //note order switched....+8 then +7
            wStr = String(RFIDUser) + "-" + String(deviceID) + ", " + String(charRFID) + ", " + String(dateAndTime)  + ", " + mBytes[i+5];
